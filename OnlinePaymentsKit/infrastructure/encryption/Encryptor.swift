...
internal class Encryptor {
    ...
    func deleteRSAKey(withTag tag: String) {
        ...
        if deleteStatus != errSecSuccess {
            Logger.log("Error while deleting the key with tag \(tag): \(deleteStatus)")
        }
    }
    ...
    func encryptRSA(plaintext: [UInt8], publicKey: SecKey) throws -> [UInt8] {
        ...
        guard SecKeyIsAlgorithmSupported(publicKey, .encrypt, algorithm) else {
            ...
            throw EncryptionError(message: EncryptionError.algorithmNotSupported)
        }

        var error: Unmanaged<CFError>?
        guard let encryptedData = SecKeyCreateEncryptedData(publicKey, algorithm, dataToEncrypt as CFData, &error)
        else {
            if let encryptionError = error?.takeRetainedValue() {
                Logger.log("Encryption failed: \(encryptionError)")
                throw EncryptionError(message: encryptionError.localizedDescription)
            } else {
                throw EncryptionError(message: EncryptionError.encryptionFailed)
            }
        }
    ...
    func decryptRSA(ciphertext: [UInt8], privateKey: SecKey) throws -> [UInt8] {
        ...
        var error: Unmanaged<CFError>?
        guard let decryptedData = SecKeyCreateDecryptedData(privateKey, algorithm, dataToDecrypt as CFData, &error)
        else {
            if let decryptionError = error?.takeRetainedValue() {
                Logger.log("Decryption failed: \(decryptionError)")
                throw EncryptionError(message: decryptionError.localizedDescription)
            } else {
                throw EncryptionError(message: EncryptionError.decryptionFailed)
            }
        }
    ...
    func stripPublicKey(publicKey: [UInt8]) throws -> ([UInt8]) {
        ...
        for index in 0..<rsaPrefixLength where rsaPublicKeyPrefix[index] != publicKey[index] {
            ...
            throw EncryptionError(message: EncryptionError.badPublicKeyFormat)
        }
