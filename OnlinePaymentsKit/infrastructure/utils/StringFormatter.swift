...
public class StringFormatter: NSObject {
    ...
    @objc public override init() {
        guard let decimalRegex = try? NSRegularExpression(pattern: "[0-9]"),
            ...
            let upperAlphaRegex = try? NSRegularExpression(pattern: "[A-Z]")
        else {
        ...
        if match.isEqual("{{") {
            ...
            while stringIndex.pointee < string.count && maskIndex < match.count {
                ...
                if copyFromMask.pointee {
                    ...
                    if maskChar.isEqual("9")
                        && (decimalRegex.numberOfMatches(in: stringChar, range: range) > 0
                            || stringChar.uppercased() == "X")
                    {
